# all commands for the operator to execute
...
__author__ = "Eric Barr, Justin Hardy"
...
led_test = "P9_11"
SOLENOID_TEN_PERCENT = "P9_17" # change this value to whatever solenoid controls ignition flow 
SOLENOID_FULL = "P9_17" 
SOLENOID_2 = "P9_19"
SOLENOID_3 = "P9_12"
SOLENOID_4 = "P9_15"
...
GPIO.setup(SOLENOID_TEN_PERCENT, GPIO.OUT)
GPIO.setup(SOLENOID_FULL, GPIO.OUT)
...
GPIO.setup(SOLENOID_8, GPIO.OUT)
GPIO.setup(SOLENOID_9, GPIO.OUT)

# high closes the valves
GPIO.output(SOLENOID_TEN_PERCENT, GPIO.HIGH)
GPIO.output(SOLENOID_FULL, GPIO.HIGH)
GPIO.output(SOLENOID_2, GPIO.HIGH)
GPIO.output(SOLENOID_3, GPIO.HIGH)
GPIO.output(SOLENOID_4, GPIO.HIGH)
GPIO.output(SOLENOID_5, GPIO.HIGH)
GPIO.output(SOLENOID_6, GPIO.HIGH)
GPIO.output(SOLENOID_7, GPIO.HIGH)
GPIO.output(SOLENOID_8, GPIO.HIGH)
GPIO.output(SOLENOID_9, GPIO.HIGH)
...
# looks like where you choose to chart 
...
# not sure what the 2-way are, type of solenoid?
def enable_2way():
    global STATES
...
def ten_percent_open():
    ...
    GPIO.output(SOLENOID_TEN_PERCENT, GPIO.LOW)
...
def full_flow_close():
    ...
    GPIO.output(SOLENOID_FULL, GPIO.HIGH)                                                            # STILL NEED TO ADD ALL SOLENOIDS ON THE LINES
...
def ignition():
    if msg.demand("Are you sure you want to start ignition? [yes/no]") == 'yes':
        ...
        mainlox_open()
        ignitor_on()
        sleep(2)                                                                                        # TEST FOR IGNITION INSTEAD OF SLEEP FOR 2 SECONDS 
        full_flow_open()
        msg.tell("BOOM")
    else:
